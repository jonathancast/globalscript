...
\newcommand\rcast{\crkwd{\rhd}}
\newcommand\lcast{\crkwd{\lhd}}

\hscommand\kind[3]{#1 \vdash #2 :: #3}
\hscommand\cotype[3]{#1 \vdash #2 :: #3}
...
A coercion represents a legal representation-preserving cast from one type to another.
A coercion can be applied to an expression using the form \<e \rcast co\> or \<co \lcast e\>,
which have equal precedence but opposite associativity.

Coercion variables have their own namespace.

Coercion types are considered normal types, of kind \<\crkwd{co}\>;
the judgment form for coercions is thus \<\cotype\Gamma{co}\tau\>.

In addition to the standard rules about type synonyms, polymorphic types, etc., the following base types are allowed for coercions:
\begin{itemize}
    \item \<\Gamma\sequent \tau_0\cong\tau_1 :: \crkwd{co}\> - isomorphism; \<\tau_0\> and \<\tau_1\>,
        which must have the same kind,
        have exactly the same set of representations.
\end{itemize}

\section{Definition Coercions}

\begin{prooftree}
    \AxiomC{\<\abstype\;t\;\overline{\alpha_i}_{i=0}^{n-1}=\tau[\overline{\alpha_i}_{i=0}^{n-1}]\in\Gamma\>}
    \UnaryInfC{
        \<\cotype\Gamma{\crdefn\;t}{\forall\;\overline{\beta_i}_{i=0}^{n-1}.\;\tau[\overline{\beta_i}_{i=0}^{n-1}]\cong{}t\;\overline{\beta_i}_{i=0}^{n-1}}\>
    }
\end{prooftree}

A definition coercion, \<\crdefn t\>, represents a cast from the definition of the abstract type \<t\>, which must be in scope, to \<t\>.
Any arguments following a definition coercion are automatically parsed as type arguments, for convenience.

\section{Type Applications}

\begin{prooftree}
    \AxiomC{\<\cotype\Gamma{co}{\forall\;\alpha :: \kappa.\;\tau_0[\alpha]}\>}
    \AxiomC{\<\kind\Gamma{\tau_1}\kappa\>}
    \BinaryInfC{\<\cotype\Gamma{co\;(\type\;\tau_1)}{\tau_0[\tau_1]}\>}
\end{prooftree}
