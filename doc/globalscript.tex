...
\newcommand\sequence[1]{\left.\overline{#1}\right.}
...
\begin{equation}\label{eqn:var-judgment}
    \Gamma \sequent v\;\sequence{\tau_{0,i} :: \kappa_i}_{i=0}^n\;\sequence{a_i :: \tau_{1,i}}_{i=0}^m :: \tau,
\end{equation}
where
\begin{itemize}
    \item $\sequence{\tau_{0,i} :: \kappa_i}_{i=0}^n$ is a sequence of \defn{implicit type arguments} and their kinds,
    \item $\sequence{a_i :: \tau_{1,i}}_{i=0}^m$ is a sequence of \defn{implicit value arguments} and their types, and
...
\begin{prooftree}
    \AxiomC{\<'x :: \sequence{\forall\;'\alpha_i :: \kappa_i.}_{i=0}^n\tau_1[\sequence{\alpha_i}_{i=0}^n] \in \Gamma\>}
    \AxiomC{\<\implicit\; 'x \not\in \Gamma\>}
    \AxiomC{$\sequence{\Gamma \sequent \tau_{0,i} :: \kappa_i}_{i=0}^n$}
    \TrinaryInfC{$\Gamma \sequent x \sequence{\tau_i :: \kappa_i}_{i=0}^n :: \tau_1[\sequence{\tau_{0,i}}_{i=0}^n]$}
\end{prooftree}
...
\begin{prooftree}
    \AxiomC{\<'x :: \sequence{\forall'\alpha_i :: \kappa_i.}_{i=0}^n \sequence{\tau_{1,i}[\sequence{\alpha_j}_{j=0}^n] \to}_{i=0}^m \tau_2[\sequence{\alpha_i}_{i=0}^n] \in \Gamma\>}
    \AxiomC{\<\implicit\; 'x = \sequence{e_i}_{i=0}^m \in \Gamma\>}
    \AxiomC{$\sequence{\Gamma \sequent \tau_{0,i} :: \kappa_i}_{i=0}^n$}
    \TrinaryInfC{\<\Gamma \sequent x \sequence{\tau_{0,i} :: \kappa_i}_{i=0}^n \sequence{e_i :: \tau_{1,i}[\sequence{\tau_{0,j}}_{j=0}^n]}_{i=0}^m :: \tau_2[\sequence{\tau_{0,i}}_{i=0}^n]\>}
\end{prooftree}
...
\begin{prooftree}
    \AxiomC{$\Gamma \sequent v \; \tau_{0,0} :: \kappa_0, \sequence{\tau_{0,i} :: \kappa_i}_{i=1}^n \sequence{e_i :: \tau_{1,i}}_{i=0}^m :: \tau$}
    \UnaryInfC{\<\Gamma \sequent v\_(\type\;\tau_{0,0}) \sequence{\tau_{0,i} :: \kappa_i}_{i=1}^n \sequence{e_j :: \tau_{1,i}}_{i=0}^m :: \tau\>}
\end{prooftree}

\subsection{Explicit Applications}

Explicit expression applications replace the implicit value arguments,
negating them and removing them from the expression.
\begin{prooftree}
    \AxiomC{$\Gamma \sequent v \sequence{\tau_{0,i} :: \kappa_i}_{i=0}^{n-1} e_0 :: \tau_{1,0}, \sequence{e_i :: \tau_{1,i}}_{i=1}^{m-1} :: \tau$}
    \AxiomC{$\Gamma \sequent e :: \tau_{1,0}$}
    \BinaryInfC{
        $\Gamma \sequent v\_e \sequence{\tau_{0,i} :: \kappa_i}_{i=0}^{n-1} \sequence{e_i :: \tau_{1,i}}_{i=1}^{m-1} :: \tau$
    }
\end{prooftree}
...
\begin{prooftree}
    \AxiomC{$\Gamma \sequent v \; \overline{\tau_i :: \kappa_i} \sequence{e_i :: \tau_{1,i}}_{i=0}^{m-1} :: \tau$}
    \AxiomC{$\sequence{\Gamma \sequent e_i :: \tau_{1,i}}_{i=0}^{m-1}$}
    \BinaryInfC{$\Gamma \sequent v :: \tau$}
\end{prooftree}
