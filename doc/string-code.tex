...
This represents the boxed un-lifted sum \<"\prod\left\langle\overline{f_i ty_i;}_{i=0}^{n-1}\right\rangle\>.
...
This represents the un-boxed un-lifted sum \<"u\prod\left\langle\overline{f_i ty_i;}_{i=0}^{n-1}\right\rangle\>.
...
{
    static gsinterned_string gssymtyproduct, gssymtyubproduct;

    while ((n = gsgrabline(pos, chan, line, fields)) > 0) {
        if (gssymceq(parsedline->directive, gssymtyproduct, gssymtypeop, ".typroduct")) {
            if (*fields[0])
