...
gsglobal_gslib_dir()
{
    ...
    if (n >= FILE_NAME_SIZE_LIMIT) {
        globalscript[FILE_NAME_SIZE_LIMIT - 1] = 0;
...
gsreadfile(char *filename, char *relname, int skip_docs, int *is_doc, int is_ags, struct gsfile_symtable *symtable)
{
    ...
    */
    ...
    if (!strcmp(calculus_version, "0.2")) {
        static int should_disable_string_code_0_2, should_disable_string_code_0_2_initialized;

        if (!should_disable_string_code_0_2_initialized) {
            struct uxio_ichannel *chan;
            char buf[0x10];
            long n;
            if (!(chan = gsbio_envvar_iopen("SHOULD_DISABLE_STRING_CODE_0_2"))) {
                should_disable_string_code_0_2_initialized = 1;
                goto decided_on_disabling;
            }
            if ((n = gsbio_get_contents(chan, buf, 0x10)) < 0)
                gsfatal("Error reading $SHOULD_DISABLE_STRING_CODE_0_2: %r")
            ;
            should_disable_string_code_0_2 = n > 0;
            should_disable_string_code_0_2_initialized = 1;
            if (gsbio_device_iclose(chan) < 0) gsfatal("Could not close $SHOULD_DISABLE_STRING_CODE_0_2 fd: %r");
        }
    decided_on_disabling:
        if (should_disable_string_code_0_2) gsfatal("%s: Illegally old calculus string code 0.2", pos.real_filename);
    }
...
static int gsparse_thunk_alloc_op(uint, struct gsparse_input_pos *, struct gsparsedline *, int, char **, long);
...
gsparse_expr_ops(struct gsparse_input_pos *pos, gsparsedfile *parsedfile, struct gsparsedline *codedirective, struct uxio_ichannel *chan, char *line, char **fields)
{
    ...
    while (
        gsparse_thunk_alloc_op(parsedfile->features, pos, parsedline, 0, fields, n)
...
gsparse_default(struct gsparse_input_pos *pos, gsparsedfile *parsedfile, struct uxio_ichannel *chan, char *line, char **fields)
{
    ...
    while ((n = gsgrabline(parsedfile->features, pos, chan, line, fields)) > 0) {
        ...
        if (gsparse_thunk_alloc_op(parsedfile->features, pos, parsedline, 0, fields, n)) {
